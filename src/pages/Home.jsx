...
import MenuContentSwitcher from '../Componentes/MenuContentSwitcher/MenuContentSwitcher';
...
export default function Home() {
    const [activeOptionBarMenu, setActiveOptionBarMenu] = useState('Inicio');

    const handleOptionClickBarMenu = (option) => {
        setActiveOptionBarMenu(option);
    };
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      >
        <Box sx={{ overflow: 'auto', backgroundColor:"#22427C" , height: '100%', color: 'white'}}>
              ...
              <div key={text}>
                ...
                <Slide direction="down" in={activeOption === text} mountOnEnter unmountOnExit>
                    <List component="div" disablePadding sx={{ marginLeft: '20px',  transformOrigin: 'top'  }}>
                      ...
                      <ListItem key={subIndex} disablePadding onClick={() => handleSubMenuOptionClick(subOptionText+text)}>
                        <ListItemButton>
                          <ListItemIcon sx={{ color: 'white' }}>
          ...
          <div style={{ textAlign: 'center', position: 'absolute', bottom: '20px', left: '50%', transform: 'translateX(-50%)' }}>
            <Button variant="outlined" sx={{ width: 200, fontSize:'1rem', color: 'white', borderColor: 'white'}}>Logout</Button>
          </div>
        </Box>
      </Drawer>
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        <Toolbar> 
          <TopBarMenu handleOptionClick={handleOptionClickBarMenu} /> 
        </Toolbar>
        <Divider/>
        <MenuContentSwitcher selectedOption={activeOptionBarMenu} />
      </Box>
    </Box>
  );
}
