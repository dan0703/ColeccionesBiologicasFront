...
const VISIBLE_FIELDS = ['idEjemplar','nombreCientifico', 'familia', 'nombreLocal', 'fechaColecta', 'nombreColector', 'protegido', 'Etiqueta'];
...
export default function GenerateTag() {
  ...
  const handleGenerateTagClick = (idEjemplar) => {
    idEjemplar = parseInt(idEjemplar, 10);
    const url = `/Tag?idEjemplar=${idEjemplar}`; 
  ...
  const columns = VISIBLE_FIELDS.map(field => {
    if (field === 'protegido') {
      ...
      return {
        ...
        flex: 1,
        renderCell: (params) => (
          <Button variant="contained" onClick={() => handleGenerateTagClick(params.row.idEjemplar)}>
            Etiqueta
          </Button>
        ),
      };
